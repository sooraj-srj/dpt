@extends('web.layouts.template')
@section('content')
    <div class="list-img-div"></div>
    <div class="details-booking-div">
        <div class="booking-inn-div">
            <div class="container">
                <div class="col-md-3">
                    <div class="price-decoration block-after-indent  screen-only">
                        <!--<div class="price-decoration__value"> <i class="fa fa-tag"></i> <span class="amount"><small>AED </small>580</span> <span class="amount"><small>USD </small>159</span> </div>-->
                        <div class="price-decoration__label"><h4>Tours & Safaries</h4></div>
                    </div>
                    <div class="clearfix"></div>
                    <div class="left-package-list">
                        <ul id="mainNav" class="list-product-ul">
                            <li class="mainNavItem">
                                <div id="navItem1">City Tours</div>
                            </li>
                            <li class="mainNavItem">
                                <div id="navItem2">Desert Safari</div>
                            </li>
                            <li class="mainNavItem">
                                <div id="navItem3">Luxuary Tours</div>
                            </li>
                            <li class="mainNavItem">
                                <div id="navItem4">Ferrari World</div>
                            </li>
                            <li class="mainNavItem">
                                <div id="navItem5">Dinner Cruise</div>
                            </li>
                        </ul>
                    </div>
                    <div class="clearfix"></div>
                    <br>
                    <div class="widget banners">
                        <div class="banner">
                            @include('web.common.tripadvisor-widget')

                        </div>
                    </div>
                </div>
                <div class="col-md-9">
                    <div class="row">
                        <div class="list-div-style-1">

                            <div class="title title--big title--center title--underline title--decoration-bottom-center">
                                <h3 class="title__primary">Select Your Emirates </h3>
                            </div>


                            <div class="col-md-4">
                                <div class="grid">
                                    <figure class="effect-marley">
                                        <img src="images/desti/fill.jpg" class="img-responsive" alt="img11"/>
                                        <figcaption>
                                            <h2><span>Dubai</span></h2>
                                            <p>View More</p>
                                            <a href="{{url('plan')}}">View more</a>
                                        </figcaption>
                                    </figure>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="grid">
                                    <figure class="effect-marley">
                                        <img src="images/desti/fill2.jpg" class="img-responsive" alt="img11"/>
                                        <figcaption>
                                            <h2><span>ABU DHABI</span></h2>
                                            <p>View More</p>
                                            <a href="{{url('plan')}}">View more</a>
                                        </figcaption>
                                    </figure>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="grid">
                                    <figure class="effect-marley">
                                        <img src="images/desti/fill3.jpg" class="img-responsive" alt="img11"/>
                                        <figcaption>
                                            <h2><span>SHARJAH</span></h2>
                                            <p>View More</p>
                                            <a href="{{url('plan')}}">View more</a>
                                        </figcaption>
                                    </figure>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="grid">
                                    <figure class="effect-marley">
                                        <img src="images/desti/fill4.jpg" class="img-responsive" alt="img11"/>
                                        <figcaption>
                                            <h2><span>FUJEIRAH</span></h2>
                                            <p>View More</p>
                                            <a href="{{url('plan')}}">View more</a>
                                        </figcaption>
                                    </figure>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="grid">
                                    <figure class="effect-marley">
                                        <img src="images/desti/fill5.jpg" class="img-responsive" alt="img11"/>
                                        <figcaption>
                                            <h2><span>AL AIN</span></h2>
                                            <p>View More</p>
                                            <a href="{{url('plan')}}">View more</a>
                                        </figcaption>
                                    </figure>
                                </div>
                            </div>

                            <div class="col-md-4">
                                <div class="grid">
                                    <figure class="effect-marley">
                                        <img src="images/desti/fill2.jpg" class="img-responsive" alt="img11"/>
                                        <figcaption>
                                            <h2><span>Ras Al Khaimah</span></h2>
                                            <p>View More</p>
                                            <a href="#">View more</a>
                                        </figcaption>
                                    </figure>
                                </div>
                            </div>

                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="clearfix"></div>
@endsection